...
const ActiveSubscriptions = () => {
    ...
    const data = details?.activeSubscriptions;

    const card = (details: any, index: any) => {
        return (
            <Flex key={index} justifyContent={"space-between"} color={"blackAlpha.700"} py={{ base: "2cqw", md: "1cqw" }} px={{ base: "6cqw", md: "3cqw" }} border={"2px solid"} borderLeft={"5px solid"} borderColor={"blue.100"} borderLeftColor={"blue.400"} borderRadius={10}>
    ...
    return (
        <Flex h={"100%"} gap={"2cqw"} flexDir={"column"} p={{ base: "2cqw" }} borderRadius={10} bg={"white"} >
            <Flex justifyContent={"space-between"} alignItems={"center"}>
                <Text color={"blackAlpha.800"} fontWeight={"600"} fontSize={{ base: "5cqw", md: "2.5cqw" }}>Active Subscriptions</Text>
            ...
            <Flex gap={"2cqw"} flexDir={"column"} flexGrow={1} >
                ...
                {data?.length === 0 && <Flex fontWeight={"bold"} color={"gray.300"} fontSize={{ base: "6cqw", md: "3cqw" }} flexGrow={1} alignItems={"center"} justifyContent={"center"} >No Active Subscriptions</Flex>}
            </Flex>
        </Flex>
    )
}
